...
//	}
    ...
    @Test
    fun `should return flights sorted by airline when both suppliers return flights and 'sortBy' is airline`(): Unit = runBlocking {
        // arrange
        val crazyAirResponse = """
			[
			  {
        ...
        assertThat(flightSearchResponses).isInstanceOf(List::class.java)
        assertThat(flightSearchResponses?.size).isEqualTo(4)
        assertThat(flightSearchResponses?.get(0)?.airline).isEqualTo("British Airways")
        assertThat(flightSearchResponses?.get(1)?.airline).isEqualTo("Emirates")
        assertThat(flightSearchResponses?.get(2)?.airline).isEqualTo("Lufthansa")
        assertThat(flightSearchResponses?.get(3)?.airline).isEqualTo("Turkish Airlines")
    }
